...
static sgx_status_t SGX_CDECL sgx_ecall_secure_save_record(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_ecall_secure_save_record_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_ecall_secure_save_record_t* ms = SGX_CAST(ms_ecall_secure_save_record_t*, pms);
	ms_ecall_secure_save_record_t __in_ms;
	if (memcpy_s(&__in_ms, sizeof(ms_ecall_secure_save_record_t), ms, sizeof(ms_ecall_secure_save_record_t))) {
		return SGX_ERROR_UNEXPECTED;
	}
	sgx_status_t status = SGX_SUCCESS;
	const char* _tmp_name = __in_ms.ms_name;
	size_t _len_name = __in_ms.ms_name_len ;
	char* _in_name = NULL;
	...
	ecall_secure_save_record((const char*)_in_name, (const char*)_in_diagnosis);
...
SGX_EXTERNC const struct {
	...
	1,
	{
		{(void*)(uintptr_t)sgx_ecall_secure_save_record, 0, 0},
	}
};
